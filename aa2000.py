...
def sondear(conn: sqlite3.Connection, aeropuerto: str = AEROPUERTO,
...
def resumen(conn: sqlite3.Connection, aeropuerto: str = AEROPUERTO) -> dict:
    """Que hay acumulado, con los ceros distinguidos de los vacios."""
    def uno(sql, *a):
        return conn.execute(sql, a).fetchone()[0]
    d = {"aeropuerto": aeropuerto}
    d["total"] = uno("SELECT COUNT(*) FROM vuelo_oficial WHERE aeropuerto=?", aeropuerto)
    for mov, nombre in (("D", "partidas"), ("A", "arribos")):
        d[nombre] = uno("SELECT COUNT(*) FROM vuelo_oficial WHERE aeropuerto=? "
                        "AND movimiento=?", aeropuerto, mov)
        d[f"{nombre}_con_real"] = uno(
            "SELECT COUNT(*) FROM vuelo_oficial WHERE aeropuerto=? AND movimiento=? "
            "AND real IS NOT NULL", aeropuerto, mov)
    d["con_matricula"] = uno("SELECT COUNT(*) FROM vuelo_oficial WHERE aeropuerto=? "
                             "AND matricula IS NOT NULL", aeropuerto)
    # Cero y vacio se cuentan APARTE: ver _pasajeros().
    d["con_pasajeros"] = uno("SELECT COUNT(*) FROM vuelo_oficial WHERE aeropuerto=? "
                             "AND pasajeros IS NOT NULL AND pasajeros > 0", aeropuerto)
    d["pasajeros_en_cero"] = uno("SELECT COUNT(*) FROM vuelo_oficial WHERE aeropuerto=? "
                                 "AND pasajeros = 0", aeropuerto)
    d["suma_pasajeros"] = uno("SELECT COALESCE(SUM(pasajeros),0) FROM vuelo_oficial "
                              "WHERE aeropuerto=? AND movimiento='D'", aeropuerto)
    fila = conn.execute(
        "SELECT MIN(programada_epoch), MAX(programada_epoch), MIN(primera_vez), "
        "MAX(ultima_vez) FROM vuelo_oficial WHERE aeropuerto=?", (aeropuerto,)).fetchone()
    d["desde_epoch"], d["hasta_epoch"] = fila[0], fila[1]
    d["primer_sondeo"], d["ultimo_sondeo"] = fila[2], fila[3]
    return d


def operaciones(conn: sqlite3.Connection, aeropuerto: str = AEROPUERTO,
    ...
    """
    sql = "SELECT * FROM vuelo_oficial WHERE aeropuerto = ?"
    args: list = [aeropuerto]
    if movimiento:
        sql += " AND movimiento = ?"
...
def main() -> int:
    ...
    try:
        ...
        while True:
            ...
            print(f"[{marca}] sondeando {a.aeropuerto} ...")
